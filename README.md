...
> **Colab import bootstrap.** The notebooks add `/content/CPS/src` to the live kernel path after installation. This is required because a newly created editable-install `.pth` file is not reprocessed automatically by an already-running kernel. No runtime restart is required.
